...
(async () => {
  ...
  banner.innerHTML = `
    ...
    <span>${esc(t("picker.clickElement"))}</span>
    <span
      style="
        margin-left:auto;
  ...
  function showNotification(message, selector, colors) {
    ...
    notification.innerHTML = `
      <div>${esc(message)}</div>
      ${selector ? `<div style="font-size:10px;opacity:0.8;font-family:monospace;margin-top:4px;word-break:break-all">${esc(selector)}</div>` : ""}
    `;
