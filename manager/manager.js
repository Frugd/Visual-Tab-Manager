...
async function loadDomains() {
  ...
  dom.hostSelect.innerHTML = `<option value="">${esc(t("manager.allDomains"))}</option>`;
...
function createCard(thumb, index) {
  ...
  image.alt = titleText;
  titleElement.innerHTML = state.searchQuery
    ? highlight(titleText, state.searchQuery)
    : esc(titleText);
...
async function loadSelectorsList() {
  ...
  if (map.size === 0) {
    dom.selectorsList.innerHTML = `<div class="items-empty">${esc(t("manager.noDomainSettingsConfigured"))}</div>`;
    return;
  }

  const entries = [...map.entries()].sort((left, right) => left[0].localeCompare(right[0]));

  dom.selectorsList.innerHTML = entries.map(([host, data]) => {
    const keywords = data.keywords.join(", ");
    return `
      <div class="item" data-host="${esc(host)}">
        <span class="item-host" style="flex:1">${esc(host)}</span>
        <code class="item-value" style="flex:2">${esc(data.selector)}</code>
        <span class="item-value" style="flex:1; color:var(--text-muted); font-size:12px">${esc(keywords)}</span>
        <div class="item-actions">
          <button class="btn-icon-sm" data-action="edit" title="${esc(t("manager.edit"))}">✏️</button>
          <button class="btn-icon-sm danger" data-action="delete" title="${esc(t("manager.delete"))}">🗑️</button>
        </div>
      </div>
    `;
  }).join("");
...
function highlight(text, query) {
  ...
  const safeQuery = escapeRegExp(esc(query));
